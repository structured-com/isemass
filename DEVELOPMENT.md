...
`isemass` is a Python CLI package for mass Cisco ISE related operations. The current implemented
routine is `isemass coa`, which reads MAC addresses from a text file, normalizes and deduplicates
them, asks for confirmation, then performs Cisco ISE CoA Reauth API requests concurrently.

The `isemass swauth` command exists as a placeholder for future network device SSH reauthentication
work. It should not be treated as implemented.
...
`isemass init` writes the packaged template at `src/isemass/templates/settings.toml` into the
platformdirs config directory. The config file is optional.
...
When changing CLI behavior, update `tests/test_cli.py`. When changing parsing, request handling,
result classification, or JSON serialization, update `tests/test_coa.py`.
...
- Prefer small, direct helpers over broad abstractions until `swauth` is implemented.
