...
from isemass.swauth import (
    ...
    SwauthCredentialValidationError,
    SwauthCsvError,
    first_switch_address,
    load_switch_mac_groups,
    process_switch,
    run_swauth_requests,
    validate_swauth_credentials,
)
...
def test_first_switch_address_uses_cleaned_input_order() -> None:
    assert (
        first_switch_address(
            {
                "first-switch.example.com": ["aa:bb:cc:dd:ee:ff"],
                "second-switch.example.com": ["00:11:22:33:44:55"],
            }
        )
        == "first-switch.example.com"
    )
...
def test_validate_swauth_credentials_connects_disconnects_and_sends_no_commands(
    tmp_path: Path,
) -> None:
    calls = []
    ...
    assert calls == [
        {
            ...
            "username": "switch-user",
            "password": "switch-pass",
            "ssh_config_file": str(tmp_path / "ssh_config"),
        }
    ]
...
def test_validate_swauth_credentials_failure_raises_without_password(tmp_path: Path) -> None:
    def fail_connect(**kwargs):
        raise RuntimeError("authentication failed")

    with pytest.raises(SwauthCredentialValidationError) as exc_info:
        validate_swauth_credentials(
            switch_address="10.1.1.1",
            username="switch-user",
            password="switch-pass",
    ...
    assert "10.1.1.1" in message
    assert "Please validate user/pass combination is correct" in message
    assert "Validate the first switch hostname/ip in the input CSV is valid" in message
    assert "switch-pass" not in message
...
def test_run_swauth_requests_calls_completion_callback_once_per_switch(tmp_path: Path) -> None:
    completed_switches = []
    ...
    results = list(
        run_swauth_requests(
            ...
            max_workers=2,
            ssh_config_file=tmp_path / "ssh_config",
            connect_handler=lambda **kwargs: FakeConnection(default_show_output=NO_SESSION_TEXT),
            on_switch_complete=completed_switches.append,
        )
    )

    assert len(results) == 2
    assert set(completed_switches) == {"10.1.1.1", "10.1.1.2"}
    assert len(completed_switches) == 2
